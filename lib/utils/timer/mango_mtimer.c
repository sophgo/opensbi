...
static inline u32 mango_cluster_id(struct mango_mtimer_data *mt, u64 hartid)
{
	return hartid / mt->cores_per_cluster;
}

static inline u32 mango_core_id(struct mango_mtimer_data *mt, u64 hartid)
{
	return hartid % mt->cores_per_cluster;
}
...
{
	unsigned int clusterid = mango_cluster_id(mt, hartid);
	unsigned int coreid = mango_core_id(mt, hartid);

	return (void *)(mt->mtimecmp_addr + CLINT_MTIMER_OFFSET +
			  clusterid * mt->cluster_mtimer_offset + coreid * 8);
}
...
static int mango_mtimer_add_regions(unsigned long addr, unsigned long size)
{
	...
	while (pos < end) {
		rsize = pos & (MTIMER_REGION_ALIGN - 1);
		if (rsize)
			rsize = 1UL << sbi_ffs(pos);
		else
			rsize = ((end - pos) < MTIMER_REGION_ALIGN) ?
				(end - pos) : MTIMER_REGION_ALIGN;
